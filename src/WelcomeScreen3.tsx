...
const SERVER_URL = 'https://easygobackend.onrender.com';
...
// // Updated Frontend: src/WelcomeScreen.tsx
// import React, { useState, useCallback, useRef, useEffect } from 'react';
// import {
//   View,
//   Text,
...
// import LinearGradient from 'react-native-linear-gradient';
// import Icon from 'react-native-vector-icons/FontAwesome';
// // Assuming you have this type definition in '../App'
// import { RootStackParamList } from '../App'; 

// // Type definition for navigation, helps with autocompletion
// type NavigationProp = NativeStackNavigationProp<RootStackParamList>;
...
// /**
//  * Helper function to call the backend API.
//  * It attempts to connect to the local server first,
//  * and if that fails, it falls back to the live server.
...
//     return await axios.post(`${LOCAL_URL}${endpoint}`, data, { timeout });
//   } catch (err) {
//     // Fallback to the live server if the local one fails
//     console.warn(`Local backend failed for ${endpoint}, falling back to live server.`);
//     return await axios.post(`${SERVER_URL}/api/auth${endpoint}`, data, { timeout });
//   }
// };
...
//   // Using a ref to store the confirmation object, which doesn't trigger re-renders
//   const confirmRef = useRef<any>(null);
//   const navigation = useNavigation<NavigationProp>();

//   // UseEffect hook to set the auth language and restore a previous session
//   useEffect(() => {
//     // Setting the language code for Firebase Auth
//     auth().setLanguageCode('en-IN'); 

//     // Asynchronously restores a previous verification session
//     const restoreVerification = async () => {
//       const verificationId = await AsyncStorage.getItem('verificationId');
//       const storedPhone = await AsyncStorage.getItem('phoneNumber');
//       if (verificationId && storedPhone) {
//         confirmRef.current = {
//           confirm: (otp: string) => auth().signInWithCredential(
...
//   // UseEffect hook for the resend OTP countdown timer
//   useEffect(() => {
//     let timer: NodeJS.Timeout;
//     if (resendCountdown > 0) {
//       setResendAvailable(false);
...
//   // A memoized function to validate the Indian phone number format
//   const isValidPhoneNumber = useCallback((phone: string) => /^[6-9]\d{9}$/.test(phone), []);
  
//   /**
//    * Initiates the Firebase phone authentication process.
//    * @param {string} phone - The mobile number to send the OTP to.
//    */
...
//     } catch (error: any) {
//       // This is the specific error you are seeing. It's a Firebase security feature.
//       if (error.code === 'auth/too-many-requests') {
//         Alert.alert('Too Many Attempts', 'We have blocked all requests from this device due to unusual activity. Try again later.');
//       } else if (error.code === 'auth/invalid-phone-number') {
//         Alert.alert('Invalid Phone Number', 'The phone number format is invalid or it is not a test number.');
//       } else {
//         Alert.alert('Error', error.message || 'Failed to send OTP.');
...
//   /**
//    * Confirms the OTP entered by the user and handles backend verification.
//    */
//   const verifyOTP = useCallback(async () => {
//     if (!code) {
//       Alert.alert('Error', 'Please enter the OTP.');
...
//       setLoading(true);
//       await confirmRef.current.confirm(code);
//       // OTP verified, now check backend
//       const response = await callBackend('/verify-phone', { phoneNumber: mobileNumber });
...
//         navigation.navigate('Screen1');
//       } else if (response.data.success && response.data.newUser) {
//         // New user, navigate to Screen1 with flag
//         await AsyncStorage.removeItem('verificationId');
//         await AsyncStorage.removeItem('phoneNumber');
//         navigation.navigate('Screen1', { isNewUser: true, phone: mobileNumber });
//       } else {
...
//                 {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Verify OTP</Text>}
//               </TouchableOpacity>
//               <TouchableOpacity style={styles.resendButton} onPress={() => sendOTP(mobileNumber)} disabled={loading || !resendAvailable}>
//                 <Text style={[styles.resendText, !resendAvailable && styles.resendDisabledText]}>
//                   {resendAvailable ? 'Resend OTP' : `Resend in ${resendCountdown}s`}
//                 </Text>
//               </TouchableOpacity>
